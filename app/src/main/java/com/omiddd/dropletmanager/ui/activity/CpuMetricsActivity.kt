...
class CpuMetricsActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val vcpus = intent.getIntExtra(EXTRA_VCPUS, 1)
        ...
        setContent {
            ...
            DropletManagerTheme(useDarkTheme = dark) {
                LaunchedEffect(dropletId) { if (dropletId > 0) viewModel.loadCpuMetrics(dropletId) }
                val cpu by viewModel.cpu.collectAsState()
                val scroll = rememberScrollState()
                Scaffold(
                    topBar = {
                        TopAppBar(
                            title = { Text("CPU Usage – $dropletName") },
                            navigationIcon = {
                    ...
                    LaunchedEffect(cpu.error) { if (cpu.error != null) errorOverlay = cpu.error }
                    ...
                    Column(
                        ...
                        val points = cpu.points.map { normalizeCpuWithVcpus(it.value, vcpus) }
                        if (points.isNotEmpty()) {
                            val last = points.last()
                            Text("Last: ${formatVal(last)}%")
                            LabeledSparkline(points, MaterialTheme.colorScheme.primary, "%")
                        } else if (cpu.isLoading) {
                            Text("Loading…")
                        } else {
                            Text("No data")
                        }
