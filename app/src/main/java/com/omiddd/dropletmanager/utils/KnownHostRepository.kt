...
class KnownHostRepository(context: Context) {
    ...
    private fun createEncryptedPrefsWithRecovery(fileName: String): SharedPreferences {
        return try {
            ...
            CrashReporter.recordNonFatal(t, mapOf("where" to "KnownHostRepository.init", "file" to fileName))
            runCatching {
    ...
    fun removeHostKey(host: String, port: Int) {
        prefs.edit().remove(prefKeyForHost(host, port)).apply()
    }
}
